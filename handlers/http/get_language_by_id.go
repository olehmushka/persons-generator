...
func (h *handlers) GetLanguageByID(w http.ResponseWriter, r *http.Request) {
	var (
		ctx = r.Context()
		id  = ExtractIDFromPath(r, "id")
	)

	lang, err := h.languageSrv.ReadLanguageByID(ctx, id)
	if err != nil {
		http_server_tools.SendErrorResp(ctx, w, err)
		return
	}
	data, err := serializeLanguage(lang)
	if err != nil {
		http_server_tools.SendErrorResp(ctx, w, err)
		return
	}
	respJSON, err := json.Marshal(GetLanguageByIDResponse{
		Data: data,
	})
	if err != nil {
		http_server_tools.SendErrorResp(ctx, w, err)
