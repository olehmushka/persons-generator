...
func getAmounts(personsAmount *int, malePersonsAmount *int, femalePersonsAmount *int) (int, int, int, error) {
	...
	if personsAmount != nil && malePersonsAmount == nil && femalePersonsAmount == nil {
		if *personsAmount < 1 {
			return 0, 0, 0, wrapped_error.NewBadRequestError(nil, fmt.Sprintf("persons_amount can not be less than 1 (persons_amount=%d)", *personsAmount))
		}
		if *personsAmount == 1 {
			return 1, 1, 0, nil
		}
	...
	if personsAmount != nil && malePersonsAmount != nil && femalePersonsAmount == nil {
		...
		if personsA < malePersonsA {
			return 0, 0, 0, wrapped_error.NewBadRequestError(nil, fmt.Sprintf("persons_amount can not be less than male_persons_amount (persons_amount=%d, male_persons_amount=%d)", personsA, malePersonsA))
		}
	...
	if personsAmount == nil && malePersonsAmount != nil && femalePersonsAmount == nil {
		if *malePersonsAmount < 1 {
			return 0, 0, 0, wrapped_error.NewBadRequestError(nil, fmt.Sprintf("male_persons_amount can not be less than 1 (male_persons_amount=%d)", *malePersonsAmount))
		}
	...
	if personsAmount == nil && malePersonsAmount != nil && femalePersonsAmount != nil {
		...
		if *femalePersonsAmount < 1 {
			return 0, 0, 0, wrapped_error.NewBadRequestError(nil, fmt.Sprintf("female_persons_amount can not be less than 1 (female_persons_amount=%d)", *femalePersonsAmount))
		}
	...
	if personsAmount != nil && malePersonsAmount == nil && femalePersonsAmount != nil {
		...
		if personsA < femalePersonsA {
			return 0, 0, 0, wrapped_error.NewBadRequestError(nil, fmt.Sprintf("persons_amount can not be less than female_persons_amount (persons_amount=%d, female_persons_amount=%d)", personsA, femalePersonsA))
		}
	...
	if personsAmount != nil && malePersonsAmount != nil && femalePersonsAmount != nil {
		...
		if malePersonsA+femalePersonsA != personsA {
			return 0, 0, 0, wrapped_error.NewBadRequestError(nil, fmt.Sprintf("sum of male_persons_amount and female_persons_amount must be equal persons_amount (persons_amount=%d, male=persons_amount=%d, female_persons_amount=%d)", personsA, malePersonsA, femalePersonsA))
		}
