...
func (h *handlers) UpdateCultureLanguage(w http.ResponseWriter, r *http.Request) {
	var (
		ctx    = r.Context()
		id     = ExtractIDFromPath(r, "id")
		langID = ExtractIDFromPath(r, "lang_id")
	)

	if err := h.cultureSrv.UpdateCultureLanguage(ctx, id, langID); err != nil {
